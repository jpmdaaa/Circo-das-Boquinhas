...
namespace TouchScript.Editor.Gestures.Base
{
    internal class TransformGestureBaseEditor : GestureEditor
    {
        public static readonly GUIContent TYPE = new("Transform Type",
            "Specifies what gestures should be detected: Translation, Rotation, Scaling.");

        public static readonly GUIContent TYPE_TRANSLATION = new("Translation", "Dragging with one ore more fingers.");
        ...
        public static readonly GUIContent MIN_SCREEN_POINTS_DISTANCE = new("Min Points Distance (cm)",
            "Minimum distance between two points (clusters) in cm to consider this gesture started. Used to prevent fake touch points spawned near real ones on cheap multitouch hardware to mess everything up.");
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (EditorGUILayout.ToggleLeft(TYPE_TRANSLATION,
                    (typeValue & (int)TransformGestureBase.TransformType.Translation) != 0, GUILayout.Width(100)))
                newType |= (int)TransformGestureBase.TransformType.Translation;
            EditorGUI.indentLevel--;
            if (EditorGUILayout.ToggleLeft(TYPE_ROTATION,
                    (typeValue & (int)TransformGestureBase.TransformType.Rotation) != 0, GUILayout.Width(70)))
