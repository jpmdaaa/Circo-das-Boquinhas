...
namespace TouchScript.Editor.Gestures
{
    ...
    internal class GestureEditor : UnityEditor.Editor
    {
        ...
        private static readonly GUIContent TEXT_ADVANCED_HEADER = new("Advanced", "Advanced properties.");
        private static readonly GUIContent DEBUG_MODE = new("Debug", "Turns on gesture debug mode.");

        private static readonly GUIContent USE_SEND_MESSAGE = new("Use SendMessage",
            "If you use UnityScript or prefer using Unity Messages you can turn them on with this option.");

        private static readonly GUIContent SEND_STATE_CHANGE_MESSAGES = new("Send State Change Messages",
            "If checked, the gesture will send a message for every state change. Gestures usually have their own more specific messages, so you should keep this toggle unchecked unless you really want state change messages.");

        private static readonly GUIContent SEND_MESSAGE_TARGET = new("Target",
            "The GameObject target of Unity Messages. If null, host GameObject is used.");

        private static readonly GUIContent COMBINE_TOUCH_POINTS = new("Combine Touch Points",
            "When several fingers are used to perform a tap, touch points released not earlier than <CombineInterval> seconds ago are used to calculate gesture's final screen position.");

        private static readonly GUIContent COMBINE_TOUCH_POINTS_INTERVAL =
            new("Combine Interval (sec)", COMBINE_TOUCH_POINTS.tooltip);

        private static readonly GUIContent REQUIRE_GESTURE_TO_FAIL = new("Require Other Gesture to Fail",
            "Gesture which must fail for this gesture to start.");

        private static readonly GUIContent LIMIT_TOUCHES = new("Limit Touches", "");
        ...
        protected virtual void OnEnable()
        {
            ...
            friendlyGesturesList.drawElementCallback += (rect, index, active, focused) =>
            {
                ...
                EditorGUI.LabelField(rect, string.Format("{0} @ {1}", gesture.GetType().Name, gesture.name),
                    GUIElements.BoxLabelStyle);
            };
        ...
        {
            EditorGUI.BeginChangeCheck();
            var expanded = GUIElements.BeginFoldout(friendlyGestures.isExpanded,
                new GUIContent(string.Format("Friendly gestures ({0})", friendlyGestures.arraySize),
                    TEXT_FRIENDLY_HEADER));
            if (EditorGUI.EndChangeCheck()) friendlyGestures.isExpanded = expanded;
            if (expanded)
            {
                GUILayout.BeginVertical(GUIElements.FoldoutStyle);
        ...
        private void drawGestureList(SerializedProperty prop, Action<SerializedProperty, Gesture> addGesture)
        {
            ...
            var dropArea = GUILayoutUtility.GetRect(0.0f, 50.0f, GUIElements.BoxStyle,
                    ...
                    if (dropArea.Contains(Event.current.mousePosition))
                    {
                        ...
                        foreach (var obj in DragAndDrop.objectReferences)
                            if (obj is GameObject)
                            {
                                ...
                                foreach (var gesture in gestures) addGesture(prop, gesture);
                            }
            ...
            if (indexToRemove > -1) removeFriendlyGestureAt(prop, indexToRemove);
        }
        ...
        private void addFriendlyGesture(SerializedProperty prop, Gesture value)
        {
            ...
            so.Update();
            var p = so.FindProperty(FRIENDLY_GESTURES_PROP);
