...
namespace TouchScript.Editor
{
    ...
    {
        private static readonly GUIContent DISPLAY_DEVICE = new("Display Device",
            "Display device properties where such parameters as target DPI are stored.");

        private static readonly GUIContent CREATE_CAMERA_LAYER = new("Create Camera Layer",
            "Indicates if TouchScript should create a CameraLayer for you if no layers present in a scene. This is usually a desired behavior but sometimes you would want to turn this off if you are using TouchScript only to get touch input from some device.");

        private static readonly GUIContent CREATE_STANDARD_INPUT = new("Create Standard Input", "");
        ...
        private static readonly GUIContent SEND_MESSAGE_TARGET = new("SendMessage Target",
            "The GameObject target of Unity Messages. If null, host GameObject is used.");

        private static readonly GUIContent SEND_MESSAGE_EVENTS =
            new("SendMessage Events", "Which events should be sent as Unity Messages.");

        private static readonly GUIContent LAYERS_HEADER =
            new("Touch Layers", "Sorted array of Touch Layers in the scene.");
        ...
        public override void OnInspectorGUI()
        {
            ...
            var newDevice =
                EditorGUI.ObjectField(r, instance.DisplayDevice as Object, typeof(IDisplayDevice), true) as
                    IDisplayDevice;
