...
namespace TouchScript.Editor.Layers
{
    ...
    internal sealed class FullscreenLayerEditor : UnityEditor.Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (EditorGUI.EndChangeCheck()) instance.Type = (FullscreenLayer.LayerType)type.enumValueIndex;

            if (type.enumValueIndex == (int)FullscreenLayer.LayerType.Camera)
            {
                EditorGUI.BeginChangeCheck();
                EditorGUILayout.PropertyField(camera);
                if (EditorGUI.EndChangeCheck()) instance.Camera = camera.objectReferenceValue as Camera;
            }
        }
    }
}