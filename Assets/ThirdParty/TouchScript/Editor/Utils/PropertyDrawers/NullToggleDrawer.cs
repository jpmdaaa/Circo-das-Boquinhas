    ...
    internal sealed class NullToggleDrawer : PropertyDrawer
    {
        ...
        public override float GetPropertyHeight(SerializedProperty property, GUIContent label)
        {
            ...
            if (property.propertyType == SerializedPropertyType.ObjectReference &&
                property.objectReferenceValue != null) return 16 * 3 + 2 * 2;
            return 16 * 2 + 2;
        }
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            else
            {
                ...
                EditorGUI.LabelField(new Rect(position.x + 14, position.y + 18, 40, 16),
                    new GUIContent("Value", label.tooltip));
                position = new Rect(position.x + 54, position.y + 18, position.width - 54, 16);
                switch (property.propertyType)
                {
                    case SerializedPropertyType.ObjectReference:
                        var objValue = EditorGUI.ObjectField(position, GUIContent.none, property.objectReferenceValue,
                        ...
                        if (objValue != null)
                        {
                            position.y += 18;
                            position.width -= 18;
                            EditorGUI.LabelField(position, string.Format("of type {0}", objValue.GetType().Name),
                                GUI.skin.FindStyle("ShurikenModuleTitle"));
                        }

                        break;
                    case SerializedPropertyType.Integer:
                        var intValue = EditorGUI.IntField(position, GUIContent.none, property.intValue);
                        if (EditorGUI.EndChangeCheck()) property.intValue = intValue;
                        break;
                    case SerializedPropertyType.Float:
                        var floatValue = EditorGUI.FloatField(position, GUIContent.none, property.floatValue);
