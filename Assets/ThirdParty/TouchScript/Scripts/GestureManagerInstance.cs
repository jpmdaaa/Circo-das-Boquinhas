...
namespace TouchScript
{
    ...
    internal sealed class GestureManagerInstance : MonoBehaviour, IGestureManager
    {
        ...
        // Upcoming changes
        private readonly List<Gesture> gesturesToReset = new(20);
        ...
        private readonly Dictionary<Transform, List<TouchPoint>> targetTouches = new(10);

        // Dictionary<Gesture, List<ITouch>> - touch points sorted by gesture
        private readonly Dictionary<Gesture, List<TouchPoint>> gestureTouches = new(10);
        private readonly List<Gesture> activeGestures = new(20);

        private static readonly ObjectPool<List<Gesture>> gestureListPool = new(10,
            () => new List<Gesture>(20), null, l => l.Clear());

        private static readonly ObjectPool<List<TouchPoint>> touchListPool = new(20,
            () => new List<TouchPoint>(10), null, l => l.Clear());

        private static readonly ObjectPool<List<Transform>> transformListPool = new(10,
        ...
        {
            if (includeSelf) getEnabledGesturesOnTarget(target, outputList);

            var count = target.childCount;
            for (var i = 0; i < count; i++) getHierarchyBeginningWith(target.GetChild(i), outputList, true);
        }
        ...
        private bool recognizeGestureIfNotPrevented(Gesture gesture)
        {
            ...
            for (var i = 0; i < count; i++)
            {
                ...
                {
                    if (canPreventGesture(gesture, otherGesture)) gesturesToFail.Add(otherGesture);
                }
            }
            ...
            if (canRecognize)
            {
                ...
                for (var i = 0; i < count; i++) failGesture(gesturesToFail[i]);
            }
