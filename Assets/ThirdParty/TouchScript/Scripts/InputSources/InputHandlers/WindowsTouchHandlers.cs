...
namespace TouchScript.InputSources.InputHandlers
{
    ...
    {
        private readonly Tags mouseTags;
        private readonly Tags penTags;
        private readonly Tags touchTags;
        ...
        {
            var pointerId = LOWORD(wParam.ToInt32());

            var pointerInfo = new POINTER_INFO();
            if (!GetPointerInfo(pointerId, ref pointerInfo)) return;
            ...
            switch (msg)
            {
                case WM_POINTERDOWN:
                    ...
                    winToInternalId.Add(pointerId,
                        beginTouch(new Vector2((p.X - offsetX) * scaleX, Screen.height - (p.Y - offsetY) * scaleY),
                            tags, true).Id);
                    break;
                case WM_POINTERUP:
    ...
    {
        private readonly int touchInputSize;

        /// <inheritdoc />
        public Windows7TouchHandler(Tags tags, Func<Vector2, Tags, bool, TouchPoint> beginTouch,
            Action<int, Vector2> moveTouch, Action<int> endTouch, Action<int> cancelTouch) : base(tags, beginTouch,
        ...
        private void decodeWin7Touches(IntPtr wParam, IntPtr lParam)
        {
            ...
            if (!GetTouchInputInfo(lParam, inputCount, inputs, touchInputSize)) return;
            ...
            for (var i = 0; i < inputCount; i++)
            {
                ...
                if ((touch.dwFlags & (int)TOUCH_EVENT.TOUCHEVENTF_DOWN) != 0)
                {
                    ...
                    winToInternalId.Add(touch.dwID,
                        beginTouch(new Vector2((p.X - offsetX) * scaleX, Screen.height - (p.Y - offsetY) * scaleY),
                            tags, true).Id);
                }
    ...
    public abstract class WindowsTouchHandler : IDisposable
    {
        ...
        protected Tags tags;
        protected Dictionary<int, int> winToInternalId = new();

        /// <summary>
        ///     Initializes a new instance of the <see cref="WindowsTouchHandler" /> class.
        /// </summary>
        /// <param name="beginTouch">
        ///     A function called when a new touch is detected. As
        ///     <see cref="InputSource.beginTouch(Vector2)" /> this function must accept a Vector2 position of the new touch and
        ///     return an instance of <see cref="TouchPoint" />.
        /// </param>
        /// <param name="moveTouch">
        ///     A function called when a touch is moved. As <see cref="InputSource.moveTouch" /> this function
        ///     must accept an int id and a Vector2 position.
        /// </param>
        /// <param name="endTouch">
        ///     A function called when a touch is lifted off. As <see cref="InputSource.endTouch" /> this
        ///     function must accept an int id.
        /// </param>
        /// <param name="cancelTouch">
        ///     A function called when a touch is cancelled. As <see cref="InputSource.cancelTouch" /> this
        ///     function must accept an int id.
        /// </param>
        public WindowsTouchHandler(Tags tags, Func<Vector2, Tags, bool, TouchPoint> beginTouch,
            Action<int, Vector2> moveTouch, Action<int> endTouch, Action<int> cancelTouch)
        {
            this.tags = tags;
        ...
        protected void initScaling()
        {
            ...
            var scale = Mathf.Max(Screen.width / (float)width, Screen.height / (float)height);
