...
namespace TouchScript.InputSources.InputHandlers
{
    ...
    public class TouchHandler : IDisposable
    {
        ...
        ///     A function called when a new touch is detected. As <see cref="InputSource.beginTouch" /> this
        ///     function must accept a Vector2 position of the new touch and return an instance of <see cref="TouchPoint" />.
        /// </param>
        ...
        ///     function must accept an int id.
        /// </param>
        public TouchHandler(Tags tags, Func<Vector2, Tags, bool, TouchPoint> beginTouch, Action<int, Vector2> moveTouch,
            Action<int> endTouch, Action<int> cancelTouch)
        {
            this.tags = tags;
        ...
        public bool HasTouches => touchesNum > 0;
        ...
        private readonly Func<Vector2, Tags, bool, TouchPoint> beginTouch;
        private readonly Action<int, Vector2> moveTouch;
        private readonly Action<int> endTouch;
        private readonly Action<int> cancelTouch;

        private readonly Tags tags;
        private readonly Dictionary<int, TouchState> systemToInternalId = new();
                ...
                switch (t.phase)
                {
                    case TouchPhase.Began:
                        if (systemToInternalId.TryGetValue(t.fingerId, out touchState) &&
                            touchState.Phase != TouchPhase.Canceled)
                        {
        ...
        {
            foreach (var touchState in systemToInternalId)
                if (touchState.Value.Phase != TouchPhase.Canceled)
                    internalCancelTouch(touchState.Value.Id);
