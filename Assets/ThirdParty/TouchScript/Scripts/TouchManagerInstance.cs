...
namespace TouchScript
{
    ...
    internal sealed class TouchManagerInstance : DebuggableMonoBehaviour, ITouchManager
    {
        ...
        public IDisplayDevice DisplayDevice
        {
            get
            {
                if (displayDevice == null) displayDevice = ScriptableObject.CreateInstance<GenericDisplayDevice>();
        ...
        /// <inheritdoc />
        public IList<TouchLayer> Layers => new List<TouchLayer>(layers);
        ...
        /// <inheritdoc />
        public int NumberOfTouches => touches.Count;

        /// <inheritdoc />
        public IList<TouchPoint> ActiveTouches => new List<TouchPoint>(touches);
        ...
        private List<TouchLayer> layers = new(10);
        private int layerCount;
        private readonly List<IInputSource> inputs = new(3);
        private int inputCount;

        private readonly List<TouchPoint> touches = new(30);
        private readonly Dictionary<int, TouchPoint> idToTouch = new(30);

        // Upcoming changes
        private readonly List<TouchPoint> touchesBegan = new(10);
        private readonly HashSet<int> touchesUpdated = new();
        private readonly HashSet<int> touchesEnded = new();
        private readonly HashSet<int> touchesCancelled = new();

        private static readonly ObjectPool<TouchPoint> touchPointPool = new(10, null, null,
            t => t.INTERNAL_Reset());

        private static readonly ObjectPool<List<TouchPoint>> touchPointListPool = new(2,
            () => new List<TouchPoint>(10), null, l => l.Clear());

        private static readonly ObjectPool<List<int>> intListPool = new(3, () => new List<int>(10), null,
            l => l.Clear());
        ...
        {
            TouchPoint touch;
            if (idToTouch.TryGetValue(id, out touch)) touch.InputSource.CancelTouch(touch, @return);
        }
        ...
        private void updateUpdated(List<int> points)
        {
            ...
            for (var i = 0; i < count; i++) touches[i].INTERNAL_ResetPosition();
