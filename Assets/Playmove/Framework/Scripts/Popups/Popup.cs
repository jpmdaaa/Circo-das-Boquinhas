...
namespace Playmove.Framework.Popups
{
    ...
    {
        private static readonly Dictionary<string, Popup> _pool = new();

        public PlaytableEvent<Popup> OnClosed = new();

        [SerializeField] private TextMeshProUGUI _title;
        [SerializeField] private TextMeshProUGUI _message;
        [SerializeField] private Transform[] _buttonsSpot;
        ...
        public static Popup Open(string title, string message, PopupButton positiveButton, PopupButton negativeButton,
            UnityAction<Popup> onClosed = null)
        {
            return Open(AssetsCatalog.GameObject_Popup2Button, title, message, onClosed, positiveButton,
                negativeButton);
        }
            ...
            UnityAction<Popup> onClosed = null, params PopupButton[] buttons)
        {
            ...
            {
                popup = Instantiate(Data.GetAsset<GameObject>(popupAssetName), PopupCanvas, false)
                    .GetComponent<Popup>();
            }
        ...
        public Popup Open(string title, string message, PopupButton[] buttons)
        {
            ...
            for (var i = 0; i < Mathf.Min(buttons.Length, _buttonsSpot.Length); i++)
            {
                var uniButton = _buttonsSpot[i].GetComponentInChildren<Button>();
                if (uniButton == null)
