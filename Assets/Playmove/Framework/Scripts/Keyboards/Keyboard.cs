...
namespace Playmove.Framework.Keyboards
{
    ...
    public class Keyboard : MonoBehaviour
    {
        ...
        /// </summary>
        public static PlaytableEvent OnOpen = new();

        /// <summary>
        ///     Event fired when keyboard method Close is called
        /// </summary>
        public static PlaytableEvent OnClose = new();

        /// <summary>
        ///     Event fired when button Confirm is pressed
        /// </summary>
        public static PlaytableEventString OnConfirm = new();

        /// <summary>
        ///     Event fired when button Cancel is pressed
        /// </summary>
        public static PlaytableEvent OnCancel = new();

        /// <summary>
        ///     Event fired when any Text on the keyboard is changed
        /// </summary>
        public static PlaytableEventString OnTextChanged = new();

        /// <summary>
        ///     Event fired when cursor position is changed
        /// </summary>
        public static PlaytableEventInt OnCursorChangedPosition = new();

        /// <summary>
        ///     Event fired when user is using ValidText = true and user tries to confirm an invalid text
        /// </summary>
        public static PlaytableEventString OnTriedToConfirmBadText = new();

        [SerializeField] private GameObject _display;

        [Header("Caps Lock")] [SerializeField] private Color _capsLockActivated = Color.white;

        [SerializeField] private Color _capsLockDeactivated = new(0.72f, 0.47f, 0.12f);

        [Header("Mute")] [SerializeField] private Sprite _muteActivated;

        [SerializeField] private Sprite _muteDeactivated;
        ...
        private KeyboardConfiguration Configuration
        {
            get
            {
                ...
                {
                    var configAsset = Localization.GetAsset<TextAsset>("keyboardConfiguration");
        ...
        /// </summary>
        public static void Open(UnityAction<string> onConfirm = null, UnityAction onCancel = null,
            bool hideFader = true)
        {
        ...
        public static void Open(KeyboardProperties properties, UnityAction<string> onConfirm = null,
            UnityAction onCancel = null, bool hideFader = true)
        {
            if (IsOpen) return;
                ...
                case KeyType.Confirm:
                    if (ValidateText)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                AudioManager.StartAudio(AudioChannel.Voice,
                                    AssetsCatalog.AudioClip_BlockedFiltersMessage).Play();
        ...
        private void LoadPage(KeyboardPage page)
        {
            ...
            for (var i = keyIndex; i < KeysLetter.Count; i++)
            {
            ...
            KeysAction[KeyType.Confirm].Letter = string.IsNullOrEmpty(Properties.ConfirmLabel)
                ? Configuration.Confirm
                : Properties.ConfirmLabel;
            KeysAction[KeyType.Cancel].Letter = string.IsNullOrEmpty(Properties.CancelLabel)
                ? Configuration.Cancel
                : Properties.CancelLabel;
        ...
        private void DisableKeysAction(string text, int cursorPosition)
        {
            ...
            if (cursorPosition == 0)
            {
                ...
                {
                    var keyLetter = KeysLetter.Find(key => key.Letter == specialChars[i].ToString());
        ...
        private void PlayAudio(string clipName)
        {
            ...
            var clip = Localization.GetAsset<AudioClip>(clipName);
            if (clip == null) return;
        ...
        public bool IsCapsLock
        {
            ...
            set
            {
                ...
                foreach (var key in KeysLetter)
                    key.Letter = _isCapsLock ? key.Letter.ToUpper() : key.Letter.ToLower();
            }
        }
