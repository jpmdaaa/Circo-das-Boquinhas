    ...
    public class Player : VmItem<AlunoVm>, IPlayerThumbnail, IDatabaseItem, ITrash
    {
        ...
        public override AlunoVm GetVm()
        {
            return new AlunoVm
            {
                Id = Id,
                Nome = Name,
                AlunoTurmas = Classrooms.Select(classroom => new AlunoTurmaVm(Id, classroom.Id, classroom.GetVm()))
                    .ToList(),
                AlunoGuid = GUID,
                Lixeira = Deleted,
                Excluido = Deleted,
                ...
                ElementosAluno = Elements.Select(element => new ElementosAlunoVm(Id, element.Id, element.GetVm()))
                    .ToList()
            };
        }
        ...
        {
            return
                $"{GUID} => Name: {Name}; Classes: {string.Join(",", Classrooms.Select(classroom => classroom.Name))};" +
                $" IsVillian: {IsVillain}; \nAvatar: {Avatar}";
        }

        private void LoadSprite(Sprite spriteCache, string spritePath, int width, int height,
            AsyncCallback<Sprite> completed)
        {
            if (spriteCache != null)
            {
                ...
                if (width > 0 || height > 0)
                    spriteCache = spriteCache.Scaled(width, height);
        ...
        [NonSerialized] private Sprite _thumbnailSpriteCache;

        [NonSerialized] private Sprite _avatarSpriteCache;
        ...
        public void GetThumbnailSprite(AsyncCallback<Sprite> completed)
        {
            GetAvatarSprite(result =>
            {
                if (result.Data != null)
                {
                    ...
                    var height = originalHeight / 1.5f;
