...
namespace Playmove.Avatars.API
{
    ...
    public static class AvatarAPI
    {
        ...
        private static List<long> _allPlayersId = new();
        ...
        /// </summary>
        public static PlaytableEvent<AsyncResult<List<Slot>>> OnSlotsMayChanged = new();
        ...
        public static bool IsAvatarLock => File.Exists(AvatarLockFilePath);
        ...
        public static bool IsLeagueLock => File.Exists(LeagueLockFilePath);

        public static List<Slot> CurrentSlots { get; set; } = new();

        public static List<Slot> SlotsPlaying
        {
            get { return CurrentSlots.Where(x => x.Players.Count > 0).ToList(); }
        }
        ...
        public static bool HasMinimumNecessaryPlayers
        {
            get
            {
                var slotsWithPlayer = CurrentSlots
        ...
        {
            return CurrentSlots.Where(x => x.Pos == slotPos).SelectMany(x => x.Players).ToList();
        }

        public static List<long> PlayersIdsFromSlot(int slotPos)
        {
            return CurrentSlots.Where(x => x.Pos == slotPos).SelectMany(x => x.PlayersId).ToList();
        }
        ...
        public static void OpenLeague()
        {
            ...
            var wait = GameObject.Find("ControlCenter").GetComponent<ControlCenter>().WaitAvatarToCloseObj;
            wait.SetActive(true);
            // ---
            Avatar.OpenLeague(() =>
            {
        ...
        {
            if (CurrentSlots != null && CurrentSlots.Count > 0 && force == false)
            {
                completed?.Invoke(new AsyncResult<List<Slot>>(CurrentSlots, string.Empty));
                return;
            }
            ...
            Slots.GetSlots(config, result =>
            {
                ...
                {
                    if (CurrentSlots.Where(x => x.Players.Count > 0).Count() > 0 ||
                        GameSettings.GUID == Playtable.LeagueGUID)
                        completed?.Invoke(result);
                    else
                        Open(confirmed => { completed?.Invoke(result); });
                }
                else
                {
                    completed?.Invoke(result);
        ...
        {
            if (CurrentSlots != null && slotPosition < CurrentSlots.Count)
            {
                completed?.Invoke(new AsyncResult<Slot>(CurrentSlots[slotPosition], string.Empty));
                return;
            }
