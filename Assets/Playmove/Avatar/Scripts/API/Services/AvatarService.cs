...
namespace Playmove.Avatars.API.Services
{
    ...
    public class AvatarService : Service<Player, AlunoVm>
    {
        ...
        {
            WebRequestWrapper.Instance.Get("/Alunos/GetAllByTurmaAndNoClass",
                new Dictionary<string, string> { { "turmaId", classroomId.ToString() }, { "semTurma", "false" } },
                result =>
                {
        ...
        {
            WebRequestWrapper.Instance.Get("/Avatar/OpenLeague",
                new Dictionary<string, string> { { "config", GameSettings.SlotsConfig.ToString() } }, result =>
                {
                    if (result.HasError)
                    {
