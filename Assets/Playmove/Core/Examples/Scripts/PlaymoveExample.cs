...
namespace Playmove.Core.Examples
{
    public class PlaymoveExample : MonoBehaviour
    {
        ...
        private void OnGetSlots(AsyncResult<List<Slot>> result)
        {
            ...
            foreach (var item in AvatarAPI.CurrentSlots)
            {
                slotPlayers[item.Pos].text = item.Id + "\n\n";
        ...
        {
            MetricsAPI.EndMatch(EndReasons.Retry, result => { OnStartMatch(); });
            // ---
            ButtonsBehavior(true, false, false, false, false);
            // ---
        }
        ...
        {
            ScoreManager.Load(result => { Debug.Log(JsonConvert.SerializeObject(ScoreManager.ranking)); });
        }
        ...
        public void SaveRanking(List<Player> players)
        {
            foreach (var item in players)
                ScoreManager.RegisterStudent(item.GUID, new Student(item.Name, Random.Range(50, 200)),
                    Random.Range(0, ScoreManager.DifficultyLevels));
            // ---
        ...
        public void DeleteRanking()
        {
            ...
            Popup.Open(Localization.GetAsset<string>(AssetsCatalog.string_Attention),
                Localization.GetAsset<string>(AssetsCatalog.string_DesejaMesmoApagarPlacar),
        ...
        private void ButtonsBehavior(bool startMatch, bool endMatch, bool startEvent, bool cancelEvent, bool endEvent,
            int slotPos = -1)
        {
            // ---
