...
namespace Tayx.Graphy
{
    ...
    internal class GraphyManagerEditor : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (m_fpsModuleInspectorToggle)
            {
                ...
                (
                    new GUIContent
                    (
                        "Time to reset min/max values",
                        "If the min/max value doesn't change in the specified time, they will be reset. This allows tracking the min/max fps in a shorter interval. \n\nSet it to 0 if you don't want it to reset."
                    ),
        ...
        {
            var path = GetMonoScriptFilePath(this);

            path = path.Split(new[] { "Assets" }, StringSplitOptions.None)[1]
                .Split(new[] { "Tayx" }, StringSplitOptions.None)[0];
