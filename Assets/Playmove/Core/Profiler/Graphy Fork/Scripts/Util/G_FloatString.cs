    ...
    public static class G_FloatString
    {
        ...
        public static void Init(float minNegativeValue, float maxPositiveValue, int decimals = 1)
        {
            ...
            if (negativeLength >= 0)
            {
                ...
                for (var i = 0; i < negativeLength; i++) negativeBuffer[i] = (-i).FromIndex().ToString(floatFormat);
            }

            if (positiveLength >= 0)
            {
                positiveBuffer = new string[positiveLength];
                for (var i = 0; i < positiveLength; i++) positiveBuffer[i] = i.FromIndex().ToString(floatFormat);
            }
        }
        ...
        public static string ToStringNonAlloc(this float value)
        {
            ...
            if (value < 0 && valIndex < negativeBuffer.Length) return negativeBuffer[valIndex];

            if (value >= 0 && valIndex < positiveBuffer.Length) return positiveBuffer[valIndex];
