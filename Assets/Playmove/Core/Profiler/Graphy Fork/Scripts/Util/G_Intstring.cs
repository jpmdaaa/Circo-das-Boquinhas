    ...
    public static class G_IntString
    {
        ...
        public static void Init(int minNegativeValue, int maxPositiveValue)
        {
            if (minNegativeValue <= 0)
            {
                ...
                for (var i = 0; i < length; i++) negativeBuffer[i] = (-i).ToString();
            }

            if (maxPositiveValue >= 0)
            {
                positiveBuffer = new string[maxPositiveValue];
                for (var i = 0; i < maxPositiveValue; i++) positiveBuffer[i] = i.ToString();
            }
        }
        ...
        {
            if (value < 0 && -value < negativeBuffer.Length) return negativeBuffer[-value];

            if (value >= 0 && value < positiveBuffer.Length) return positiveBuffer[value];
