...
namespace Playmove.Core.Bundles
{
    ...
    public static class PlaytableBundlesPath
    {
        ...
        public static string BundlesLoadPath =>
            Directory.GetParent(Application.dataPath).FullName.Replace(@"\", "/") + "/Bundles";

        /// <summary>
        ///     Path to bundles when game is run by Unity Editor
        /// </summary>
        public static string BundlesLoadPathEditor => Application.dataPath + "/Bundles";

        /// <summary>
        ///     Path to bundles assets in Unity Editor
        /// </summary>
        public static string BundlesAssetsPath => Application.dataPath + "/BundlesAssets";

        public static UnityDirInfo GetBuildDirectory(string bundleAssetsPath)
        {
            var bundleAssetsDirName = new DirectoryInfo(bundleAssetsPath).Name;
            var buildPath = bundleAssetsPath.Replace("BundlesAssets", "Bundles");
            try
            {
                if (buildPath.LastIndexOf('_') > -1)
                    buildPath = buildPath.Replace(bundleAssetsDirName,
                        bundleAssetsDirName.Substring(0, bundleAssetsDirName.IndexOf('_')));
            }
            catch
            {
            }
        ...
        public static List<string> GetExpansionPaths()
        {
            ...
            try
            {
                foreach (var path in Directory.GetDirectories(GetContextPath(BundleContext.Expansions)))
        ...
        {
            var expansionPaths = GetExpansionPaths();
            for (var i = 0; i < expansionPaths.Count; i++)
        ...
        public static List<string> GetContentsPath(string expansionPath)
        {
            ...
            {
                foreach (var path in
                         Directory.GetDirectories(GetExpansionBundlePath(expansionPath, BundleType.Content)))
                    paths.Add(path.Replace(@"\", "/"));
            }
            catch
            {
        ...
        public static List<string> GetContentsPath()
        {
            ...
            try
            {
                foreach (var path in Directory.GetDirectories(GetGlobalBundlePath(BundleType.Content)))
                    paths.Add(path.Replace(@"\", "/"));
            }
            catch
            {
        ...
        public static List<string> GetDatasPath(string expansionPath)
        {
            ...
            try
            {
                foreach (var path in Directory.GetDirectories(GetExpansionBundlePath(expansionPath, BundleType.Data)))
                    paths.Add(path.Replace(@"\", "/"));
            }
            catch
            {
        ...
        public static List<string> GetDatasPath()
        {
            ...
            try
            {
                foreach (var path in Directory.GetDirectories(GetGlobalBundlePath(BundleType.Data)))
                    paths.Add(path.Replace(@"\", "/"));
            }
            catch
            {
        ...
        public static List<string> GetLocalizationsPath(string expansionPath)
        {
            ...
            try
            {
                foreach (var path in Directory.GetDirectories(GetExpansionBundlePath(expansionPath,
                             BundleType.Localization)))
                    paths.Add(path.Replace(@"\", "/"));
            }
            catch
            {
        ...
        public static List<string> GetLocalizationsPath()
        {
            ...
            try
            {
                foreach (var path in Directory.GetDirectories(GetGlobalBundlePath(BundleType.Localization)))
                    paths.Add(path.Replace(@"\", "/"));
            }
            catch
            {
        ...
        public static List<string> GetAvailableLocalizations()
        {
            ...
            {
                var languageSplit = path.Split('/').LastOrDefault().Split('-');
