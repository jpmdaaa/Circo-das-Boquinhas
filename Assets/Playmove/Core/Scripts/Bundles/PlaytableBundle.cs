...
namespace Playmove.Core.Bundles
{
    ...
    {
        private static PlaytableBundle _instance;

        /// <summary>
        ///     This event is raised everytime a bundle is loaded passing the proggress on all bundles
        /// </summary>
        public PlaytableEventFloat OnLoading = new();

        /// <summary>
        ///     This event is raised when all bundles get loaded
        /// </summary>
        public PlaytableEvent OnLoaded = new();

        private float _bundlesLoaded;
        private int _bundlesToLoad;
        ...
        {
            Content.Reload(() => { Data.Reload(() => Localization.Reload(() => completed?.Invoke())); });
        }
        ...
        {
            var bundles = new List<Bundle>(ExpansionBundles.ContentBundles);
        ...
        {
            var bundles = new List<Bundle>(ExpansionBundles.DataBundles);
        ...
        public List<Bundle> GetLocalizationBundle(string language)
        {
            var bundles =
                new List<Bundle>(
                    ExpansionBundles.LocalizationBundles.Where(bundle => bundle.Name.ToLower() == language.ToLower()));
        ...
        private IEnumerator LoadBundlesRoutine(List<Bundle> bundles, List<string> bundlesRootPath,
            Action completedCallback)
        {
            foreach (var bundleRootPath in bundlesRootPath)
            {
                var bundlesPath = new string[0];
                try
                {
        ...
        {
            public List<Bundle> ContentBundles = new();
            public List<Bundle> DataBundles = new();
            public List<Bundle> LocalizationBundles = new();
        }
    }
}