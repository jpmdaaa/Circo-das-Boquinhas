...
namespace Playmove.Core.Bundles
{
    ...
    public static class Localization
    {
        ...
        /// </summary>
        private static readonly Dictionary<string, string> _localizedStrings = new();
        ...
        {
            var amountBundlesToReload = Bundles.Count;
        ...
        {
            return GetAsset(asset.AssetName, defaultValue);
        }
        ...
        {
            return GetAssets(asset.AssetName, defaultValue);
        }
        ...
        public static List<T> GetAssets<T>(List<T> defaultValue = default)
        {
            ...
            {
                if (typeof(T) == typeof(string))
                    return _localizedStrings.Select(keyValue =>
                    {
                        object hack = keyValue.Value;
                        return (T)hack;
                    }).ToList();
                foreach (var bundle in Bundles)
        ...
        {
            var stringsAssets = GetAssets<TextAsset>("strings");
        ...
        /// </summary>
        private static void ParseCustomStringsFile()
        {
            throw new NotImplementedException();
        }

#if UNITY_EDITOR
        private static readonly Dictionary<string, string> EDITOR_LocalizedStrings = new();
        ...
        private static void EDITOR_ParseStringsFile()
        {
            ...
            IEnumerable<string> assetsPath = Directory.GetFiles(Application.dataPath + "/BundlesAssets/",
                "strings.json", SearchOption.AllDirectories);
            foreach (var assetPath in assetsPath)
            {
                var asset = AssetDatabase.LoadAssetAtPath<TextAsset>(assetPath.Replace(Application.dataPath, "Assets"));
        ...
        private static void EDITOR_ParseCustomStringsFile()
        {
            throw new NotImplementedException();
        }
#endif
