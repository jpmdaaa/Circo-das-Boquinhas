...
namespace Playmove.Core.API
{
    ...
    public class WebRequestWrapper : MonoBehaviour
    {
        ...
        /// <param name="callback">Callback with DownloadHandler when completed</param>
        public void Get(string relativeUrl, Dictionary<string, string> parameters,
            AsyncCallback<DownloadHandler> callback)
        {
        ...
        /// <param name="parameters">Parameters you need to pass to service</param>
        /// <param name="callback">Callback with DownloadHandler when completed</param>
        public void Get(string apiVersion, string relativeUrl, Dictionary<string, string> parameters,
            AsyncCallback<DownloadHandler> callback)
        {
            var param = string.Join("&", parameters.Select(element => $"{element.Key}={element.Value}"));
        ...
        {
            var request = UnityWebRequest.Get(Playtable.Instance.APIBaseURL + apiVersion + relativeUrl);
            SetDefaultHeaders(request);
            yield return request.SendWebRequest();

            if (GlobalDebugs.Instance.DebugWebRequests)
            {
        ...
        /// <param name="callback">Callback with DownloadHandler when completed</param>
        /// <returns></returns>
        private IEnumerator PostRoutine(string apiVersion, string relativeUrl, string data,
            AsyncCallback<DownloadHandler> callback)
        {
            var request = new UnityWebRequest(Playtable.Instance.APIBaseURL + apiVersion + relativeUrl, "POST")
            {
