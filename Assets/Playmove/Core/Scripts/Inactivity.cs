...
public class Inactivity : MonoBehaviour
{
    ...
    public void GetInactiveTime()
    {
        PlaytableAPI.GetInactiveTime(result =>
        {
            if (result.Data > 0)
            {
                ...
                TouchManager.Instance.TouchesBegan += (sender, args) => { ResetTimer(); };
                ResetTimer();
            }
        });
    }
    ...
    {
        if (AvatarAPI.IsAvatarLock) return;

        ControlBoxSubPopup.CloseIfAny();
        Fader.FadeTo(0.75f, 0.5f);
        _timer = 30;
        _popup = Popup.Open(Localization.GetAsset<string>(AssetsCatalog.string_Attention),
            string.Format(Localization.GetAsset<string>(AssetsCatalog.string_Close_Game_After), "30"),
            new PopupButton(Localization.GetAsset<string>(AssetsCatalog.string_Cancel)) { Action = _ => ResetTimer() },
            new PopupButton(Localization.GetAsset<string>(AssetsCatalog.string_Fechar))
    ...
    public void StartCountdown()
    {
        ...
        _popup.UpdateMessage(
            string.Format(Localization.GetAsset<string>(AssetsCatalog.string_Close_Game_After), _timer));
        Invoke("StartCountdown", 1);
    }
