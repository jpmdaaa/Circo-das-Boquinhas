...
namespace Playmove.Core.Controls
{
    ...
    public class ControlCenter : Openable, IPointerDownHandler
    {
        ...
        [SerializeField] private ControlLanguageSelector _languageSelector;
        [SerializeField] private ControlSlider _microphoneSlider;
        [SerializeField] private ControlSlider _volumeSlider;
        [SerializeField] private List<Transform> _buttons = new();

        [Header("Buttons to be hidden in each Scene")] [SerializeField]
        private List<HideButtonsInSceneData> _buttonsToHideInScenes = new();

        private Animator _animator;

        private AvatarNotification _avatarNotification;
        ...
        private void LateUpdate()
        {
            ...
            {
                var eventObj = EventSystem.current.currentSelectedGameObject;
                if (eventObj == null)
                {
                    Close();
                    return;
        ...
        private void SceneManager_sceneLoaded(Scene scene, LoadSceneMode loadMode)
        {
            ...
            var hideButtons = _buttonsToHideInScenes.Find(item => item.SceneName == scene.name);
            if (hideButtons != null)
        ...
        public void CloseGame()
        {
            ...
            Popup.Open(Localization.GetAsset<string>(AssetsCatalog.string_Attention),
                Localization.GetAsset<string>(AssetsCatalog.string_WantToClose),
                new PopupButton(Localization.GetAsset<string>(AssetsCatalog.string_Cancel)),
                new PopupButton(Localization.GetAsset<string>(AssetsCatalog.string_Continue))
                    { Action = _ => Playtable.Instance.ForceExit() },
        ...
        public void RotateScreen()
        {
            ControlBoxSubPopup.CloseIfAny();
            Fader.FadeTo(1, 0.5f, () =>
                PlaytableAPI.RotateScreen(result => { Fader.FadeTo(0, 0.5f); })
            );
        }
