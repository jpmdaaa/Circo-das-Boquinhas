...
namespace Playmove.Core.Storages
{
    public static class TextureExtension
    {
        public static bool HasAlpha(this Texture2D texture, float percentageAlphaPixels)
        {
            ...
            var necessaryAlphaPixels = (int)(pixels.Length * percentageAlphaPixels / 100);
        ...
        public static void FloodFillArea(this Texture2D aTex, int pixelX, int pixelY, Color32 aFillColor)
        {
            ...
            var refCol = colors[pixelX + pixelY * w];
        ...
        public static void FloodFillArea(this Texture2D aTex, Texture2D comparableTex, int pixelX, int pixelY,
            Color32 aFillColor)
        {
            var w = comparableTex.width;
            var h = comparableTex.height;
            ...
            var refCol = colorsComp[pixelX + pixelY * w];
            if (refCol.r == 0 &&
                refCol.g == 0 &&
                refCol.b == 0)
                return;
            ...
            while (nodes.Count > 0)
            {
                ...
                for (int i = current.x; i < w; i++)
                {
                    var C = colorsComp[i + current.y * w];
                    if (!CompareColor32(C, refCol) ||
                        CompareColor32(C, aFillColor))
                        break;
        ...
        public static void FloodFillAreaWithTexturePattern(this Texture2D aTex, Texture2D comparableTex, int pixelX,
            int pixelY)
        {
            var w = comparableTex.width;
            var h = comparableTex.height;
            ...
            bool generateFallbackPhysicsShape = false, FilterMode mode = FilterMode.Bilinear)
        {
            ...
            return Sprite.Create(src.texture, new Rect(0, 0, src.texture.width, src.texture.height), Vector2.one / 2,
                pixelsPerUnit, extrude,
        ...
        public static Texture2D Scaled(this Texture2D src, int width, int height, bool keepAspect = true,
        ...
        /// <param name="mode">Filtering mode</param>
        public static void Scale(this Texture2D src, int width, int height, bool keepAspect = true,
            FilterMode mode = FilterMode.Bilinear)
        {
            if (keepAspect)
                KeepAspectRatio(src.width, src.height, ref width, ref height);
        ...
        private static readonly Rect rectForGPU = new(0, 0, 1, 1);
        ...
        private static void _gpu_scale(Texture2D src, int width, int height, FilterMode fmode)
        {
            ...
            var rtt = RenderTexture.GetTemporary(width, height, 16);
        ...
        private static void KeepAspectRatio(int originalWidth, int originalHeight, ref int width, ref int height)
        {
            var ratioX = (float)originalWidth / originalHeight;
            var ratioY = (float)originalHeight / originalWidth;
            if (ratioX > ratioY)
