...
namespace Playmove.Core.Audios
{
    ...
    {
        private static readonly Dictionary<AudioSource, float> _audioSources = new();
        private static AudioManager _instance;

        private static readonly Dictionary<AudioChannel, float> _oldChannelVolume = new();

        [SerializeField] private AudioMixer _mixer;
        private readonly List<AudioData> _executingAudios = new();

        private readonly Queue<AudioData> _pool = new();
        ...
        public static IAudioData StartAudio(AudioChannel channel, params string[] clipsName)
        {
            ...
            {
                var clip = Data.GetAsset<AudioClip>(clipName);
                if (clip == null)
                {
            ...
            if (clips.Count == 0)
            {
                ...
                var errorAudio = Instance.GetAudioData($"Not Found: {clipName}");
                errorAudio.Initialize(AudioChannel.Voice);
        ...
        public static IAudioData StartAudio(AudioChannel channel, params AudioClip[] clips)
        {
            if (channel == AudioChannel.Voice)
            {
                ...
                var currentVoice = Instance.GetExecutingAudioDataReference(clips[0]);
            ...
            var audioData = Instance.GetAudioData(clips[0].name);
        ...
        {
            return;
            foreach (var audio in _audioSources) Instance.StartCoroutine(Instance.FadeVolumeRoutine(audio, 1, 5f));
            _audioSources.Clear();
        }
        ...
        {
            Instance._mixer.GetFloat($"{channel}Volume", out var volume);
