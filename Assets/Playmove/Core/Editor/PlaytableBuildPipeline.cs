...
namespace Playmove.Core.Editor
{
    public class PlaytableBuildPipeline : IPreprocessBuildWithReport
    {
        ...
        public static void OnPostprocessBuild(BuildTarget target, string exeFinalPath)
        {
            ...
            var dir = new FileInfo(exeFinalPath);
            const string BUILD_FILE_NAME = "build_version_";

            // Delete old build_version file
            try
            {
                var filesInfo = dir.Directory.GetFiles("*.txt", SearchOption.TopDirectoryOnly)
            ...
            try
            {
                var buildVersion = DevKit.BuildVersion.Value;
        ...
        {
            //Set Boquinhas Build Version
            var guids = AssetDatabase.FindAssets("t:" + typeof(BoquinhasBuildVersion).Name);
            BoquinhasBuildVersion buildVersionFile;
            for (var i = 0; i < guids.Length; i++) //probably could get optimized
            {
                var path = AssetDatabase.GUIDToAssetPath(guids[i]);
                buildVersionFile = AssetDatabase.LoadAssetAtPath<BoquinhasBuildVersion>(path);
                buildVersionFile.version = "v 1.0." + DateTime.Now.Year.ToString().Substring(2) +
                                           DateTime.Now.Month + DateTime.Now.Day.ToString("00") +
                                           DateTime.Now.Hour.ToString("00") + DateTime.Now.Minute.ToString("00") +
                                           " [" + EsqueletoVersion.version + "]";
                buildVersionFile.SetDirty();
                AssetDatabase.SaveAssets();
            }
            //--

            var catalogPaths = AssetDatabase.FindAssets("t:AssetsCatalog");
            if (catalogPaths.Length > 0)
            {
                var catalog =
                    AssetDatabase.LoadAssetAtPath<AssetsCatalog>(AssetDatabase.GUIDToAssetPath(catalogPaths[0]));
                if (AssetsCatalogEditor.HasWarningInCatalog(catalog))
                {
                    if (EditorUtility.DisplayDialog("Assets Catalog Warning",
                            "Your catalog has some assets that need's your attention.", "Fix"))
            ...
            var executableName = GameSettings.ExecutableName;
            var lastBuildPath = PlaytablePrefs.Get<string>($"{DevKit.ProjectName}PlaymoveLastBuildPath");
            ...
            // Configure build
            var scenes = new List<string>(EditorBuildSettings.scenes.Where(scene => scene.enabled)
            ...
            if (dirtyBundles.Count > 0 &&
                EditorUtility.DisplayDialog("Dirty bundles",
                    $"You have {dirtyBundles.Count} dirty bundles thats need to be build.", "Build Now", "Build Later"))
            ...
            var exeInfo = new FileInfo(lastBuildPath);
        ...
        {
            var buildDir = PlaytableBundlesPath.GetBuildDirectory(bundleAssetPath);
            var dirInfo = new DirectoryInfo(bundleAssetPath);
        ...
        public static void BuildBundle(string bundleAssetsPath, string bundleBuildPath, string bundleName)
        {
            ...
            var directory = new DirectoryInfo(bundleBuildPath);
            var assetsPath = GetAssetsPathFromFolder(bundleAssetsPath);
            ...
            var expansionOrGlobal = bundleBuildPath.Contains("/Expansions/") ? "Expansions" : "Global";
        ...
        {
            var assetsPath = Directory.GetFiles(path, "*.*", SearchOption.AllDirectories);
            assetsPath = assetsPath.Where(p => !p.EndsWith(".meta")).ToArray();

            for (var i = 0; i < assetsPath.Length; i++)
            {
                assetsPath[i] = assetsPath[i].Replace("\\", "/");
                var splitted = new List<string>(assetsPath[i].Split('/'));
                var startIndex = splitted.IndexOf("Assets");
