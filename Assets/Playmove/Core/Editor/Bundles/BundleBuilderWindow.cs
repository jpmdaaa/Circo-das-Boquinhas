...
namespace Playmove.Core.Editor.Bundles
{
    ...
    {
        private GUIStyle _boxGrid3;
        private GUIStyle _btnStyle;
        private Vector2 _scroll = Vector2.zero;

        private void OnGUI()
        {
            GUILayout.BeginArea(new Rect(-6, 0, Screen.width, Screen.height - 21),
                EditorStyles.inspectorDefaultMargins);
            _scroll = GUILayout.BeginScrollView(_scroll);
            // Global Bundles
            GUILayout.BeginHorizontal();
            GUILayout.BeginVertical(GUI.skin.box);
            var contentsPath = PlaytableBundlesPath.GetContentsPath();
            var datasPath = PlaytableBundlesPath.GetDatasPath();
            var localizationsPath = PlaytableBundlesPath.GetLocalizationsPath();
            var hasGlobalBundles = contentsPath.Count > 0 || datasPath.Count > 0 || localizationsPath.Count > 0;
            if (DrawFoldout("Global Bundles",
                    hasGlobalBundles ? "Global Bundles" : "Global Bundles: None bundle found"))
            {
            ...
            GUILayout.BeginHorizontal();
            GUILayout.BeginVertical(GUI.skin.box);
            var expansionNames = PlaytableBundlesPath.GetExpansionNames();
            if (DrawFoldout("Expansion Bundles",
                    expansionNames.Count > 0 ? "Expansion Bundles" : "Expansion Bundles: None bundle found"))
                foreach (var expansionName in expansionNames)
                {
                    ...
                    {
                        DrawBundleTypeBox("expansion", BundleType.Content,
                            PlaytableBundlesPath.GetContentsPath(expansionName));
                        DrawBundleTypeBox("expansion", BundleType.Data,
                            PlaytableBundlesPath.GetDatasPath(expansionName));
                        DrawBundleTypeBox("expansion", BundleType.Localization,
                            PlaytableBundlesPath.GetLocalizationsPath(expansionName));
                    }
        ...
        {
            var state = EditorGUILayout.Foldout(GetFoldout(key), label);
        ...
        private void DrawBundleTypeBox(string globalOrExpansion, BundleType type, List<string> bundlesPath)
        {
            ...
            GUILayout.Space(10);
            GUILayout.BeginVertical();
            var label = bundlesPath.Count > 0
                ? $"{type.ToString()}: {bundlesPath.Count}"
                : $"{type.ToString()}: None bundle found";
        ...
        private void DrawBundlesBuildButtons(List<string> bundlesPath)
        {
            ...
            for (var i = 0; i < bundlesPath.Count; i++)
            {
                GUILayout.BeginHorizontal(_boxGrid3);
                for (var y = 0; y < 3 && i < bundlesPath.Count; y++)
                {
        ...
        private void DrawBuildButton(string path)
        {
            ...
            var pastGUIColor = GUI.color;
            GUI.color = GetBundleStateColor(path);

            if (GUILayout.Button(new GUIContent(directory.Name, directory.Name), _btnStyle,
                    GUILayout.Width(Screen.width * 0.3f)))
            {
