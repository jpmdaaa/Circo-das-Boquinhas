...
namespace Playmove.Core.Editor
{
    public static class BuildAssetsCatalog
    {
        ...
        private static string AssetsCatalogScriptPath
        {
            get
            {
                if (string.IsNullOrEmpty(_assetsCatalogScriptPath))
                    _assetsCatalogScriptPath = Directory.GetFiles(Application.dataPath, "AssetsCatalog.cs",
        ...
        {
            var catalogPath = AssetDatabase.FindAssets("AssetsCatalog").Select(guid =>
                AssetDatabase.GUIDToAssetPath(guid)).Where(path => path.EndsWith(".asset")).FirstOrDefault();
            var catalog = AssetDatabase.LoadAssetAtPath<AssetsCatalog>(catalogPath);
            if (catalog == null)
            {
            ...
            {
                var asset = AssetDatabase.LoadAssetAtPath<PlayAsset>(AssetDatabase.GUIDToAssetPath(guid));
                if (asset != null &&
                    catalog.Assets.Find(item => item.AssetName == asset.AssetName && item.Type == asset.Type) == null)
            ...
            var assetsThatAreInIgnoredFolders =
                catalog.Assets.Where(asset => catalog.IsFolderIgnored(asset.RelativePath)).ToList();
            ...
            {
                EditorUtility.DisplayDialog("Assets Catalog Warning",
        ...
        {
            var catalogContent = "using System.Collections.Generic;\n" +
            ...
            var assets = assetsCatalog.Assets.Where(asset => !asset.Ignore).OrderBy(asset => asset.Type).ToList();
            var fixedTags = new List<string>();
            foreach (var asset in assets)
            {
                if (string.IsNullOrEmpty(asset.Tag)) continue;
                var assetTag = $"{asset.Type}_{asset.Tag}";
                var tagRepeatedAmount = 0;
                var fixedAssetTag = assetTag;
        ...
        public static PlayAsset CreatePlayAsset(string assetTag, string assetName, string assetType,
            string unityRelativePath)
        {
            var asset = ScriptableObject.CreateInstance<PlayAsset>();
        ...
        {
            var unityAssetsPath = Directory.GetFiles(Application.dataPath, "*.*", SearchOption.AllDirectories)
            ...
            {
                var assetType = AssetDatabase.GetMainAssetTypeAtPath(path).FullName.Split('.').LastOrDefault();
                var assetTag = path.Split('/').LastOrDefault().Split('.').FirstOrDefault();
                if (catalog.IsFolderIgnored(path)) continue;

                PlayAsset asset = null;
                if (path.Contains("/Resources/"))
                {
                    var assetName = path.Substring(path.IndexOf("/Resources/") + 11).Split('.').FirstOrDefault();
        ...
        private static void CreatePlayAssetsFromLocalizationStrings(AssetsCatalog catalog)
        {
            foreach (var keyValue in Localization.EDITOR_GetLocalizedStrings())
            {
                ...
                var asset = CreatePlayAsset(keyValue.Key, keyValue.Key, "string", keyValue.Value);
                AssetDatabase.CreateAsset(asset, $"Assets/AssetsCatalog/{asset.name}.asset");
                catalog.Assets.Add(asset);
            }
        }
        ...
        {
            var pAsset = catalog.GetAsset(assetName, "string");
            if (pAsset != null)
                pAsset.RelativePath = unityRelativePath.Replace("\\", "/").Replace(Application.dataPath, string.Empty);
        }

        private static void UpdateUnityPlayAsset(AssetsCatalog catalog, string assetName, string assetType,
            string unityRelativePath)
        {
            var pAsset = catalog.GetAsset(assetName, assetType);
            if (pAsset != null)
